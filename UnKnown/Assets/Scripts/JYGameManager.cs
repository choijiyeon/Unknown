...
public class JYGameManager : MonoBehaviour
{
    ...
    public Transform m_InvincibilityMonsterRoot = null;
    public GameObject[] m_InvincibilityMonsterPos = new GameObject[1];
    ...
    void GameLoading()
    {
        ...
        for (int i = 0; i < m_InvincibilityMonsterPos.Length; i++)
        {
            DoActorLoad(JYDefines.ActorType.InvincibilityMonster, "monster02", m_InvincibilityMonsterPos[i].gameObject.transform.position, i);
        }
    }
    ...
    public void DoActorLoad(JYDefines.ActorType aActorType, string aResName, Vector3 aPos, int index = 0)
    {
        ...
        switch (aActorType)
        {
            ...
            case JYDefines.ActorType.AttackMonster:
                ...
                break;
            case JYDefines.ActorType.InvincibilityMonster:
                {
                    GameObject go = Resources.Load("Character/" + aResName) as GameObject;
                    if (null != go)
                    {
                        GameObject actor = GameObject.Instantiate(go, aPos, Quaternion.identity, m_InvincibilityMonsterRoot);
                        actor.name = index.ToString();
                        JYMonster character = actor.AddComponent<JYMonster>();
                        character.monsterType = JYMonster.MonsterType.INVINCIVILITY;
                        character.DoCreate();
                    }
                }
                break;
